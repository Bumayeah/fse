...
function fse_register_work_items_block() {
    register_block_type( 'fse/work-items', array(
        'render_callback' => function( $attributes ) {
            ...
            while ( $query->have_posts() ) {
                ...
                $items .= sprintf(
                    '<li class="group relative flex flex-col items-start">
                        %s
                        <h2 class="mt-6 text-base font-semibold text-zinc-800 dark:text-zinc-100">%s</h2>
            ...
            return sprintf(
                '<ul role="list" class="grid grid-cols-1 gap-x-12 gap-y-16 sm:grid-cols-2 lg:grid-cols-3">%s</ul>',
                $items
            );
        },
    ) );
